...
from .obstacle_map_2d import ObstacleMap, generate_random_obstacles, populate_obstacle_map
...
class Navigation2DEnv:
    def __init__(
        self, config, device=torch.device("cuda"), dtype=torch.float32, seed: int = 42
    ) -> None:
        # device and dtype
        ...
        #     map_size=(30, 30), cell_size=0.1, device=self._device, dtype=self._dtype
        # )
        self._obstacle_map = ObstacleMap(
            config, cell_size=2*config.obstacle_length, device=self._device, dtype=self._dtype
        )
        self._seed = seed
        ...
        self._start_pos = torch.tensor(
            config.start_loc, device=self._device, dtype=self._dtype
        )
        self._goal_pos = torch.tensor(
            config.goal_loc, device=self._device, dtype=self._dtype
        )
