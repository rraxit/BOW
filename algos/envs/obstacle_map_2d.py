...
class ObstacleMap:
    ...
    def __init__(
        ...
        self.x_lim = [-x_range / 2 + config.map_origin[0], x_range / 2 + config.map_origin[0]]  # [m]
        self.y_lim = [-y_range / 2 + config.map_origin[1], y_range / 2 + config.map_origin[1]]  # [m]
...
def populate_obstacle_map(
    obstacle_map: ObstacleMap,
    config
) -> None:

    for center in config.obstacles:
        obstacle_map.add_rectangle_obstacle(np.array(center), config.robot_width, config.robot_length)
        # obstacle_map.x_lim[0] = min(obstacle_map.x_lim[0], center[0] - config.robot_length)
        # obstacle_map.x_lim[1] = max(obstacle_map.x_lim[1], center[0] + config.robot_length)
        # obstacle_map.y_lim[0] = min(obstacle_map.y_lim[0], center[1] - config.robot_length)
        # obstacle_map.y_lim[1] = max(obstacle_map.y_lim[1], center[1] + config.robot_length)
